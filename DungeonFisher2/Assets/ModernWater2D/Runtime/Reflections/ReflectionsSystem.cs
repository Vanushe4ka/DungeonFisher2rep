...
namespace Water2D
{
    ...
    public class ReflectionsSystem : WaterFeatureLayerRenderer
    {
        ...
        public static ReflectionsSystem GetInstanceTopDown() 
        {
            ...
            {
                foreach (var system in FindObjectsOfType<ReflectionsSystem>()) if (system.topdown) { instanceTopDown = system; return instanceTopDown; } 
            }
        ...
        public static ReflectionsSystem GetInstancePlatformer()
        {
            ...
            {
                if (FindObjectsOfType<ReflectionsSystem>().Where(t => t.topdown == false).ToList().Count > 0) instancePlatformer = FindObjectsOfType<ReflectionsSystem>().Where(t => t.topdown == false).ToList()[0];
                else throw new Exception("Reflection System Platformer Instance couldn't be found in the scene");
            }
        ...
        [HideInInspector][SerializeField] public bool topdown;
        ...
        {
            _reflectionsSettings.originalColor.value = reflectionsSettings.originalColor.value;
            _reflectionsSettings.length.value = reflectionsSettings.length.value;
            _reflectionsSettings.color.value = reflectionsSettings.color.value;
            _reflectionsSettings.angle.value = reflectionsSettings.angle.value;
            _reflectionsSettings.alpha.value = reflectionsSettings.alpha.value;
            _reflectionsSettings.tilt.value = reflectionsSettings.tilt.value;
            _reflectionsSettings.y.value = reflectionsSettings.mirrorY.value;
        ...
        private void UpdateReflectionsShader()
        {
            ...
            else if (!Shader.IsKeywordEnabled(WaterShaderIdsREF.reflectionsTexture) && !topdown) Shader.SetGlobalTexture(WaterShaderIdsREF.reflectionsTexture2, layerRenderer.LayerTexture());
        }
